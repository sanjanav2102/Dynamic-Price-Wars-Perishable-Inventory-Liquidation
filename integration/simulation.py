...
class PriceWarSimulation:
    ...
    def run_round(self) -> None:
        ...
        state = (
            self.environment.get_market_state()
        )
        ...
        if state.remaining_inventory > 0:

            result = (
                self.negotiation_manager.negotiate(
                    "premium_grocery"
                )
            )
    ...
    def run(self) -> None:
        ...
        # Run until product expires
        # or inventory becomes zero.
        ...
        status = (
            self.environment.get_status()
        )

        print(
            f"Remaining inventory: "
            f"{status['remaining_inventory']}"
        )

        print(
            f"Expired inventory: "
            f"{status['expired_inventory']}"
        )

        print(
            f"Total units sold: "
            f"{status['metrics']['total_units_sold']}"
        )

        print(
            f"Total revenue: "
            f"₹{status['metrics']['total_revenue']:.2f}"
        )

        print(
            f"Successful deals: "
            f"{status['metrics']['successful_deals']}"
        )

        print(
            f"Failed negotiations: "
            f"{status['metrics']['failed_negotiations']}"
        )
